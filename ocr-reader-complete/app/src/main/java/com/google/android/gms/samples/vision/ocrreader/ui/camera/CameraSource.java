...
public class CameraSource {
    ...
    private Queue<Pair<byte[], Long>> frameQueue = new LinkedList<>();               // Added by rokus
    ...
    public void takePicture(ShutterCallback shutter, PictureCallback jpeg) {
        synchronized (cameraLock) {
            if (camera != null) {
                Log.d("PictureTaken", String.valueOf(counter));
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        void setNextFrame(byte[] data, Camera camera) {
            synchronized (mLock) {
                ...
                frameQueue.add(new Pair<byte[], Long>(data,  SystemClock.uptimeMillis() - mStartTimeMillis));            // added by rokus
                mPendingTimeMillis = frameQueue.peek().second;                                                           // changed by rokus
                mPendingFrameId++;
                mPendingFrameData = bytesToByteBuffer.get(frameQueue.peek().first);                                     // changed by rokus
                frameQueue.remove();                                                                                    // added by rokus
